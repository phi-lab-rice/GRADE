...
def create_dataloader(
    ...
    dji_calibrate: bool = False,
) -> DataLoader:
    """Create a dataloader for the Rice dataset.

    Split rules
    -----------
    train : Rice sequences NOT in test-rice (split.json)
    ...
    if split in ("train", "val"):
        ...
        if split == "val":
            sequences = [s for s in val_rice_seqs if s in all_rice_seqs]
        else:
            sequences = [s for s in all_rice_seqs if s not in val_rice_seqs]
