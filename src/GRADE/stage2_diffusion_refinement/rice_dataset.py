...
class RiceDataset(Dataset):
    ...
    def _discover_sequences(self, sequences: Optional[List[str]] = None) -> List[str]:
        ...
        valid = [
            ...
            and (
                not self.proc_params["use_rgb"]
                or (self.root_dir / name / self.RGB_FILENAME).exists()
            )
        ]
    ...
    def _build_index(self) -> None:
        ...
        for seq_name in self.sequences:
            ...
            if self.proc_params["use_rgb"]:
                rgb = np.load(seq_dir / self.RGB_FILENAME, mmap_mode="r")
    ...
    def _load_sequence_arrays(self, seq_name: str) -> Dict:
        if seq_name not in self._seq_arrays:
            ...
            if self.proc_params["use_rgb"]:
                arrays["rgb"] = np.load(
                    seq_dir / self.RGB_FILENAME, mmap_mode="r"
                )
