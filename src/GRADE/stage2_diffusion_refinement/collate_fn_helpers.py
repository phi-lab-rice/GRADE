...
def radar_collator(
    ...
    """Convert radar amplitude/phase to normalized spectrum."""
    if radar_phase is not None:
        radar_data = torch.stack([radar_amplitude, radar_phase], dim=1)
        radar_data = radar_data.permute(0, 1, 5, 2, 4, 3)
    else:
        radar_data = radar_amplitude

    radar_mag = radar_data[:, 0:1, :, :, :, :]
    radar_phase_ch = radar_data[:, 1:2, :, :, :, :]

    radar_mag_processed = torch.sqrt(radar_mag + 1e-8) * scale_factor
    radar_phase_processed = to_zero_one(radar_phase_ch)

    return torch.cat([radar_mag_processed, radar_phase_processed], dim=1)
...
def dji_rgb_collator(
    ...
    """Defish, calibrate, and resize a batch of DJI RGB images.
